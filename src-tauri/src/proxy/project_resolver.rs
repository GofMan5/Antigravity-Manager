...
/// Detect legacy random mock project IDs generated by old versions.
pub fn is_legacy_mock_project_id(project_id: &str) -> bool {
    ...
    let valid_adj = matches!(adj, Some("useful" | "bright" | "swift" | "calm" | "bold"));
    let valid_noun = matches!(noun, Some("fuze" | "wave" | "spark" | "flow" | "core"));
    let valid_suffix = suffix
        .map(|s| s.len() == 5 && s.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit()))
        .unwrap_or(false);

    valid_adj && valid_noun && valid_suffix
}
...
pub async fn fetch_project_id(access_token: &str) -> Result<String, String> {
    ...
    // 如果没有返回 project_id，说明账号无资格，返回错误以触发稳定兜底逻辑
